#!/bin/bash
# Qwen3-1.9B Summary Attention — 128k pretrain continuing from 64k DCP checkpoint.
#
# Two modes (controlled by RESUME_DATALOADER below):
#   - RESUME_DATALOADER=0: first launch. Load weights only from 64k DCP,
#                          start 128k training from step 0 with a fresh dataloader.
#   - RESUME_DATALOADER=1: mid-run resume. CHECKPOINT_DIR must point to this
#                          128k run's own output; weights + dataloader state both restored.
#
# Usage: bash examples/pretrain/run_pretrain_128k.sh
...
# Paths — ADJUST THESE
# ============================================
CHECKPOINT_DIR=/path/to/muse_outputs/1b9_sa_hybrid_64k   # 64k DCP on first launch; this 128k output on resume
MODEL_CONFIG=examples/pretrain/model_config/model_config_1b9_hybrid.json
DATASET_CONFIG=examples/pretrain/dataset_config/pretrain_kai_mmap_128k.json
OUTPUT_DIR=/path/to/muse_outputs/1b9_sa_hybrid_128k
...
WARMUP_STEPS=100
DECAY_STEPS=100
TOTAL_STEPS=1600          # ~50B text tokens (240*131072*1600)
CHUNK_SIZE=1024
USE_CHUNKED_CE=1          # 1 = chunked CE (memory-efficient), 0 = standard CE
RESUME_DATALOADER=1       # 0 = first launch from 64k; 1 = resume this 128k run
...
comment="pretrain_1b9_sa_hybrid_128k"
...
echo "pretrain 128k started. Logs:"
