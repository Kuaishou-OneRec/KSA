...
class TransformerDecoder(nn.Module):
    ...
    def _validate_inputs(
        ...
        if seq_len > self.max_seq_len:
            # For SA models, summary tokens inflate seq_len but position IDs
            # stay within max_seq_len (each summary token reuses the position
            # of its nearest left text token). Check input_pos instead.
            if input_pos is not None and input_pos.max() < self.max_seq_len:
                pass  # position IDs are within range, OK
            else:
                raise ValueError(
